...
| M8 | Inpainting, UI kompozicija, frame pacing, latencija | 2 tj | FPS raste, frame time ravnomjeran, HUD čist | ✅ gotovo (uz opterećenje ×0–×24 prikazani FPS 1,15–1,75×; std intervala 0,18–0,58 ms ravnomjerno naspram 3,0–4,8 ms odmah; latencija +1,2 do +6,4 ms; HUD 52,0 dB komponiran naspram 20,0 dB upečen; inpainting slike ±0,1 dB — `docs/PACING.md`). Usput nađen i popravljen bug: procjena render perioda bila je samoreferentna na brzom GPU-u, a `waitUntil` je preko `sleep_for` gubio ~10–13 ms na Windowsu; popravljeno mjerenjem `renderMs` neovisno o prezentaciji i aktivnim čekanjem umjesto uspavljivanja. |
